...
sudo coreos-installer install --copy-network \
     --ignition-url=http://$BASTION_IP:8080/pre/bootstrapbk.ign /dev/$NODE_DISK_PARTITION \
     --insecure-ignition
EOF

cat << EOF > $INSTALL_DIR/master01-installer-2.sh
sudo coreos-installer install --copy-network \
     --ignition-url=http://$BASTION_IP:8080/pre/master01.ign /dev/$NODE_DISK_PARTITION \
     --insecure-ignition
EOF

cat << EOF > $INSTALL_DIR/master02-installer-2.sh
sudo coreos-installer install --copy-network \
     --ignition-url=http://$BASTION_IP:8080/pre/master02.ign /dev/$NODE_DISK_PARTITION \
     --insecure-ignition
EOF

cat << EOF > $INSTALL_DIR/master03-installer-2.sh
sudo coreos-installer install --copy-network \
     --ignition-url=http://$BASTION_IP:8080/pre/master03.ign /dev/$NODE_DISK_PARTITION \
     --insecure-ignition
EOF

cat << EOF > $INSTALL_DIR/worker01-installer-2.sh
sudo coreos-installer install --copy-network \
     --ignition-url=http://$BASTION_IP:8080/pre/worker01.ign /dev/$NODE_DISK_PARTITION \
     --insecure-ignition
EOF

cat << EOF > $INSTALL_DIR/worker02-installer-2.sh
sudo coreos-installer install --copy-network \
     --ignition-url=http://$BASTION_IP:8080/pre/worker02.ign /dev/$NODE_DISK_PARTITION \
     --insecure-ignition
EOF
