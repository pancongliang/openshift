...
export DISK_KNAME_PATTERN="sd[a-z]"    # If the OCP version is greater than 4.18 and the node is not an SSD, the MachineConfig from https://access.redhat.com/articles/6547891 should be applied. This script only manually changes the disk rotational setting.
...
# Install version
export ODF_CHANNEL_NAME="stable-4.20"
...
# Get the version of OpenShift cluster
OCP_VERSION=$(oc get clusterversions -o jsonpath='{.items[0].status.desired.version}')
OCP_MAJOR=$(echo $OCP_VERSION | cut -d'.' -f1)
OCP_MINOR=$(echo $OCP_VERSION | cut -d'.' -f2)

# Check if the version is greater than or equal to 4.18
ssh_opts="-o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o LogLevel=ERROR -o ConnectTimeout=15 -o BatchMode=yes"
if [[ $OCP_MAJOR -ge 4 && $OCP_MINOR -ge 18 ]]; then
  for Hostname in $(oc get nodes -l "$ODF_NODES_LABEL" -o jsonpath='{.items[*].status.addresses[?(@.type=="Hostname")].address}'); do
    # Apply disk rotational settings(rotational=0) on node
    ssh $ssh_opts core@$Hostname <<EOF > /dev/null 2>&1
      echo 'ACTION=="add|change", SUBSYSTEM=="block", KERNEL=="$DISK_KNAME_PATTERN", ATTR{queue/rotational}="0"' | sudo tee /etc/udev/rules.d/99-disk.rules > /dev/null
      sudo udevadm control -R > /dev/null 2>&1
      sudo udevadm trigger > /dev/null 2>&1
EOF

    if [[ $? -eq 0 ]]; then
      echo -e "$INFO_MSG Apply disk rotational settings(rotational=0) on node $Hostname"
    else
      echo -e "$FAIL_MSG Apply disk rotational settings(rotational=0) on node $Hostname"
    fi
  done
else
  echo -e "$INFO_MSG OCP versions lower than 4.18 skip rotation settings (rotational=0)"
fi

# Create the StorageCluster resource
oc create -f - <<EOF >/dev/null 2>&1 
